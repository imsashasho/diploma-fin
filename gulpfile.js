...
// JS
function _scripts() {
  return gulp.src(pathsProd.js.src).pipe(gulp.dest(pathsProd.js.dest));
}
// IMG
function _images() {
  return gulp
    .src(pathsProd.images.src)
    .pipe(
      cache(
        imagemin(
          [
...
gulp.task(
  'prod',
  gulp.series(_clean, gulp.parallel(_templates, _fonts, _static, _scripts, _styles, _images)),
);
